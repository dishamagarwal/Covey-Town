...
export function ViewingAreaVideo({
  ...
  useEffect(() => {
    const progressChanged = (updatedTime: number) => {
      if (
        reactPlayerRef.current?.getCurrentTime() &&
        (updatedTime > reactPlayerRef.current?.getCurrentTime() + ALLOWED_DRIFT ||
          updatedTime < reactPlayerRef.current?.getCurrentTime() - ALLOWED_DRIFT)
      ) {
        reactPlayerRef.current?.seekTo(updatedTime);
      }
    };

    const playbackChanged = (playing: boolean) => {
      setIsPlaying(playing);
    };

    controller.addListener('playbackChange', playbackChanged);
    controller.addListener('progressChange', progressChanged);
    return () => {
      controller.removeListener('playbackChange', playbackChanged);
      controller.removeListener('progressChange', progressChanged);
    };
  }, [controller]);
  // In response to these events, the ViewingAreaVideo updates the ViewingAreaController's properties, and
  // uses the TownController to emit a viewing area update.
    ...
    <Container className='participant-wrapper'>
      ...
      <ReactPlayer
        ...
        onProgress={progress => {
          ...
          if (currentDuration && controller.elapsedTimeSec !== currentDuration) {
            controller.elapsedTimeSec = currentDuration;
            townController.emitViewingAreaUpdate(controller);
          }
        }}
        onPause={onPause}
