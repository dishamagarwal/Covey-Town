  ...
  registerSocketListeners() {
    ...
    this._socket.on('interactableUpdate', interactable => {
      if (isConversationArea(interactable)) {
        const oldConversationArea = this.conversationAreas.find(
          oldArea => oldArea.id == interactable.id,
        );
        if (oldConversationArea !== undefined) {
          const isEmpty = oldConversationArea.isEmpty();
          oldConversationArea.topic = interactable.topic;
          oldConversationArea.occupants = this._playersInternal.filter(occupant =>
            interactable.occupantsByID.includes(occupant.id),
          );
          const index = this._conversationAreasInternal.findIndex(area => {
            return area.id === oldConversationArea.id;
          });
          if (isEmpty !== oldConversationArea.isEmpty()) {
            const controller = new ConversationAreaController(
              oldConversationArea.id,
              oldConversationArea.topic,
            );
            if (
              this._conversationAreasInternal[index].occupants !== oldConversationArea.occupants
            ) {
              this._conversationAreasInternal[index] = controller;
            }
            if (index !== -1) {
              this._conversationAreasInternal.concat(controller);
            }
            this.emit('conversationAreasChanged', this._conversationAreasInternal);
          }
        }
      } else if (isViewingArea(interactable)) {
        const oldViewingArea = this.viewingAreas.find(oldArea => oldArea.id == interactable.id);
        if (oldViewingArea !== undefined) {
          const viewingAreaChanged = oldViewingArea !== interactable;
          const controller = new ViewingAreaController(interactable);
          if (viewingAreaChanged) {
            oldViewingArea.elapsedTimeSec = interactable.elapsedTimeSec;
            oldViewingArea.isPlaying = interactable.isPlaying;
            oldViewingArea.video = interactable.video;
            this.emitViewingAreaUpdate(controller);
          }
        }
      }
    });
...
export function useTownSettings(): { friendlyName: string; isPubliclyListed: boolean } {
  ...
  useEffect(() => {
    const updateTownSettings = (updateSetting: TownSettingsUpdate) => {
      if (updateSetting.friendlyName !== undefined) {
        setFriendlyName(updateSetting.friendlyName);
      }
      if (updateSetting.isPubliclyListed !== undefined) {
        setIsPubliclyListed(updateSetting.isPubliclyListed);
      }
    };
...
export function useActiveConversationAreas(): ConversationAreaController[] {
  const townController = useTownController();
  const [currentArea, setCurrentArea] = useState<ConversationAreaController[]>(
    townController.conversationAreas.filter(area => !area.isEmpty()),
  );
  useEffect(() => {
    const doFilter = (newAreas: ConversationAreaController[]) => {
      setCurrentArea(newAreas.filter(area => !area.isEmpty()));
    };
    townController.addListener('conversationAreasChanged', doFilter);
    return () => {
      townController.removeListener('conversationAreasChanged', doFilter);
    };
  }, [townController]);
  return currentArea;
}
...
export function usePlayers(): PlayerController[] {
  const townController = useTownController();
  const [currentPlayers, setCurrentPlayers] = useState<PlayerController[]>(townController.players);
  useEffect(() => {
    townController.addListener('playersChanged', setCurrentPlayers);
    return () => {
      townController.removeListener('playersChanged', setCurrentPlayers);
    };
  }, [townController]);
