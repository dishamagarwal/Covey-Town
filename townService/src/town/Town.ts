...
export default class Town {
  ...
  async addPlayer(userName: string, socket: CoveyTownSocket): Promise<Player> {
    ...
    socket.on('interactableUpdate', (update: Interactable) => {
      if (isViewingArea(update)) {
        ...
        const prevInteractable = this._interactables.find(conv => conv.id === update.id);
        let newArea: ViewingAreaModel;
        if (prevInteractable) {
          (prevInteractable as ViewingArea).updateModel(update as ViewingArea);
        }
      }
    });
    return newPlayer;
  ...
  public addViewingArea(viewingArea: ViewingAreaModel): boolean {
    ...
    // area.isPlaying = viewingArea.isPlaying;
    // area.video = viewingArea.video;
    // area.elapsedTimeSec = viewingArea.elapsedTimeSec
